...
function UserDetailPage() {
  ...
  // 1. ✅ Added email to the initial state
  ...
  const loadUser = async () => {
    try {
      ...
      // 2. ✅ Map email from the fetched user data to state
      setFormData({
        name: data.name,
        role: data.role,
        email: data.email, 
  ...
  const handleSave = async () => {
    try {
      // 3. ✅ Now formData.email will contain the valid email string instead of undefined
    ...
    <div className="user-detail-page">
      ...
      <div className="product-header-card">
        <div className="product-badge-row">
          <span className="product-code-badge">USER #{user.id}</span>
          <span className={`status-badge ${user.role === "ADMIN" ? "status-active" : "status-inactive"}`}>
            {user.role}
          </span>
        </div>

        <h1 className="product-title">{user.name}</h1>
          ...
          <table className="uom-table">
            <tbody>
              ...
              <tr>
                <th>Email</th>
                <td>
                  {/* 4. ✅ Render input for email when editing, or keep it readonly if you wish */}
                  {editing ? (
                    <input
                      type="email"
                      className="form-input"
